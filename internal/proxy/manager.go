...
// 全量 CF IPv4 兜底池（= CF 官方 15 段全集 ∪ cfnat-docker 的 ips-v4.txt(baipiao 源)，每个 /24 随机抽 1 IP）
// 前 15 条 = CF 官方段（https://www.cloudflare.com/ips-v4，展开 5956 个 /24，保留官方全集——
// baipiao 在其中精简掉的 642 个 /24 依然保留，无效段由 colo 探测自然淘汰）；
// 104.28.0.0/14 + 1.0.0.0/24 + 1.1.1.0/24 = baipiao 源有而官方未列的缺口：
//   - 104.28.0.0/14：CF 主力段 104.16.0.0/12 的另一半（1024 个 /24，与 104.16-104.27 同级）
//   - 1.0.0.0/24 + 1.1.1.0/24：CF DNS anycast 段（见 fallbackExhaustSlash24，全量枚举不抽样）
// fallbackBaipiaoExtraCIDRs = baipiao 源其余 644 个 /24（海外 partner/杂段，聚合为 411 条，
// 含 91.193.58.0/23、8.x、154.x 等——单段价值不高但避免遗漏，失效段由 colo 探测自然淘汰）；
// 用户自收集的非公开段（fofa 等渠道，时效性差）不入代码，走 IP 库收藏。
var fallbackCIDRs = append([]string{
...
}, fallbackBaipiaoExtraCIDRs...)

// fallbackExhaustSlash24 这几段不做"/24 随机抽 1 个"，始终枚举全部 256 个主机（对齐 docker：
// 它的 ips-v4.txt 把 1.0.0.x/1.1.1.x 写成 512 行单 IP，每轮全量进候选、约占 6.8%；
// AIO 若按 /24 随机抽样每轮只有 2 个候选，密度差 256 倍）
var fallbackExhaustSlash24 = map[string]bool{
	"1.0.0.0/24": true,
	"1.1.1.0/24": true,
}

// fallbackBaipiaoExtraCIDRs baipiao 源的海外 partner/杂段
//（644 个 /24 聚合为 411 条，2026-07-24 与 docker 的 ips-v4.txt 逐段 diff 生成）
var fallbackBaipiaoExtraCIDRs = []string{
	"5.10.244.0/22", "5.182.34.0/24", "5.182.84.0/23", "5.226.179.0/24", "5.226.181.0/24", "8.6.144.0/23",
	"8.6.146.0/24", "8.9.231.0/24", "8.10.148.0/24", "8.12.10.0/24", "8.14.199.0/24", "8.14.201.0/24",
	"8.14.202.0/23", "8.14.204.0/24", "8.17.205.0/24", "8.17.206.0/24", "8.18.50.0/24", "8.18.113.0/24",
	"8.18.194.0/23", "8.18.196.0/24", "8.19.8.0/24", "8.20.100.0/23", "8.20.103.0/24", "8.20.122.0/23",
	"8.20.124.0/23", "8.20.126.0/24", "8.20.253.0/24", "8.21.8.0/23", "8.21.10.0/24", "8.21.13.0/24",
	"8.21.110.0/23", "8.21.239.0/24", "8.23.139.0/24", "8.23.240.0/24", "8.24.87.0/24", "8.24.242.0/23",
	"8.24.244.0/24", "8.25.96.0/23", "8.25.249.0/24", "8.26.180.0/24", "8.26.182.0/24", "8.27.64.0/24",
